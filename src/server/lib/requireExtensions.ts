...
export let isDev = config.get('isDev');

/**
 * 构建信息
 */
export let buildMeta: BuildMeta | null = null;
if (!isDev) {
 ...
 * @param ext 文件类型
 */
export const assertResolver = (ext: string) => (m: NodeModule, filename: string) => {
...
export function fileResolver(file: string) {
  ...
  result = isDev ? `//${webpackHost}:${webpackPort}${file}` : `//${bsHost}/bs/${bsApp}/@${file}`;
  return result;
};

/**
 * [addRequireExt description]
 * @return {[type]} [description]
 */
export function addRequireExt() {
